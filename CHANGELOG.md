...
- **Fluid / server-side data layer.** Reuse the same `Configuration/TcaApi/` resource definitions that power the REST API to pull fully-resolved records into Fluid templates — no Extbase model, repository, or QueryBuilder query required.
  - New public facade `MaikSchneider\TcaApi\Frontend\TcaApiRepository` — inject it into a controller and call `collection('articles', order: ['created' => 'desc'], itemsPerPage: 5)`, `collectionResult(...)` (items + pagination metadata), or `find('articles', $uid)`. Returns clean PHP arrays so templates address plain properties (`{article.title}`, `{article.color.name}`, `{article.image.publicUrl}`).
  - The current frontend `SiteLanguage` is resolved automatically (language overlay applies); pass an explicit `$language` to override. The REST access roles (PUBLIC/FE_USER/OWNER) are **not** enforced on this path — server-side rendering is the integrator's responsibility — but TCA-level safety (deleted, hidden, start/endtime, storagePid, language) always applies.
  - New helper `MaikSchneider\TcaApi\Frontend\FluidArrayNormalizer` — strips the JSON-LD envelope (`@id`/`@type`/`@context`/`hydra:*`) from serializer output; public service, reusable.
- New internal API `MaikSchneider\TcaApi\DataAccess\ResourceDataProvider` (with `CollectionQuery`, `ItemQuery`, `CollectionResult` value objects) — the request-free read pipeline (filter/order validation, pagination, fetch, embed-preload, serialize) now shared by both the HTTP operation handlers and the new frontend data layer, so there is a single code path for reads.
