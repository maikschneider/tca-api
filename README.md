...
| `RangeFilter` | Comparison operators on a column (numeric, string or date) | `value` must be `['gte'=>…, 'lte'=>…, 'gt'=>…, 'lt'=>…]`. Optional `type` (`int`\|`float`\|`string`\|`date`\|`datetime`) overrides autodetection (e.g. `?filters[created_at][gte]=2024-01-01`) |
